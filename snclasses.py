...
#       pl.show()
    ...
    def gpphot(self, b, phaserange=None, fig=None, ax=None,
        ...
        if ax:
            ...
            pl.savefig("gpplots/" + self.name + "_" + b + ".gp.png", bbox_inches='tight')
            ...
            #pl.show()
            pl.savefig("gpplots/" + self.name + "_" + b + ".gpsample.png", bbox_inches='tight')
            ...
            for b in mybands:
                ...
                if plottemplate:
                    ...
                    if save:
                        ...
                        try: pl.savefig(thisname)
                        except RuntimeError: pl.show()
...
#                            pass
                ...
                pl.legend(legends[::-1], bandswdata[::-1], loc=1, ncol=1, prop={'size': 12}, numpoints=1, framealpha=0.5)
                for i in notused:
                    mybands.remove(i)
                if savepng:
                    bnds = "UBVRIriHJK.png"
        ...
        for b in self.su.bands:
            ...
            if not b in ['w1','w2','m2']:
                if sum(self.lc['photcode'] == self.su.photcodes[b][2][:2]) > 0:
                    litindx = np.where(self.lc['photcode'] == self.su.photcodes[b][2][:2])[0]
...
#            return (self.photometry[band]['mjd'][indx], self.photometry[band]['mag'][indx],self.photometry[band]['dmag'][indx])
    ...
    def formatlitsn(self, lit_lc, verbose=True):
        ...
        for b in self.su.bands:
            ...
            if not b in self.su.photcodes.iterkeys():
                continue
        ...
        if nir:
            if len(self.name) == 7:
                fileout = open(os.environ['SESNPATH'] + "/literaturedata/nirphot/" + self.name[:-1] + self.name[-1].upper() + '.dat', 'w')
            else:
